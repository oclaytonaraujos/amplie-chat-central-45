...
export function Sidebar({
  ...
  return <div className={cn("h-full flex flex-col", isMobile ? "w-64 p-3 sm:p-4" : "w-64 p-3 sm:p-4")}>
      <div ref={sidebarRef} className={cn("bg-amplie-sidebar flex-1 transition-all duration-300 ease-out relative flex flex-col rounded-2xl shadow-lg h-full transform-gpu", isMobile ? "w-full" : isCollapsed ? "w-16" : "w-full", isDragging && "transition-none")} style={{
      WebkitBackfaceVisibility: 'hidden',
      backfaceVisibility: 'hidden',
      willChange: 'transform'
    }}>
        {/* Header */}
        <div className="p-4 sm:p-6 border-b border-gray-700/30 flex-shrink-0 relative px-0">
          <div className="flex items-center justify-between">
            ...
            {!isCollapsed && <div className="flex items-center space-x-2 pr-8 sm:pr-0">
                <img src="/lovable-uploads/5b035ca2-6a1b-45db-b6e2-f291736cf358.png" alt="Logo" className="h-6 sm:h-8 w-auto" />
              </div>}
            
            {isCollapsed && <div className="w-full flex justify-center">
                <img src="/lovable-uploads/a2e5cd13-e36f-4dee-b922-d556b4ba1516.png" alt="Logo Icon" className="h-8 sm:h-10 w-8 sm:w-10 object-contain" />
              </div>}
          </div>
          
          {/* Toggle button - only show on desktop */}
          {!isMobile && <button onClick={handleToggleCollapse} className={cn("p-1.5 rounded-lg hover:bg-amplie-sidebar-hover text-gray-400 hover:text-white transition-all duration-200 touch-manipulation active:scale-95", isCollapsed ? "absolute -right-3 top-1/2 -translate-y-1/2 bg-amplie-sidebar border border-gray-700/30 shadow-lg z-10" : "absolute right-4 sm:right-6 top-1/2 -translate-y-1/2")} style={{
          touchAction: 'manipulation'
        }}>
        ...
        overscrollBehavior: 'contain'
          ...
          <nav className="space-y-1">
            ...
            return <Link key={item.href} to={item.href} onClick={handleLinkClick} className={cn("flex items-center rounded-lg transition-all duration-200 group touch-manipulation active:scale-95", isCollapsed ? "justify-center px-0 py-3 w-full" : "space-x-3 px-3 py-3", isActive ? isCollapsed ? "bg-amplie-sidebar-active text-white shadow-lg w-full" : "bg-amplie-sidebar-active text-white shadow-lg" : "text-gray-300 hover:bg-amplie-sidebar-hover hover:text-white")} style={{
              touchAction: 'manipulation'
            }}>
                  <div className={cn("p-2 rounded-lg transition-colors", isCollapsed && isActive ? "bg-transparent" : isActive ? "bg-white/20" : "bg-gray-700/30 group-hover:bg-gray-600/50")}>
                    <Icon className={cn("w-4 h-4 sm:w-5 sm:h-5", isActive ? "text-white" : item.color)} />
                  </div>
