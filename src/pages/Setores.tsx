...
export default function Setores() {
  ...
  return <div className="p-4 md:p-6 space-y-6 min-h-screen">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6">
        {filteredSetores.map(setor => <div key={setor.id} className="bg-white rounded-xl shadow-amplie p-4 md:p-6 hover:shadow-amplie-hover transition-all duration-300 min-w-0 flex flex-col">
            {/* Header do Card */}
            <div className="flex items-start justify-between mb-4 gap-2">
              ...
              <div className="flex items-center space-x-1 flex-shrink-0">
                <Button variant="ghost" size="sm" onClick={() => setSetorParaEditar(setor)} className="h-8 w-8 p-0">
                  <Edit className="w-4 h-4 text-gray-500" />
                </Button>
                <Button variant="ghost" size="sm" onClick={() => setSetorParaExcluir(setor)} className="h-8 w-8 p-0">
            ...
            {/* Descrição */}
            {setor.descricao && <div className="mb-4 min-w-0">
      ...
      {filteredSetores.length === 0 && setores.length > 0 && <div className="bg-white rounded-xl shadow-amplie p-8 md:p-12 text-center">
      ...
      {/* Empty State inicial */}
      {setores.length === 0 && <div className="bg-white rounded-xl shadow-amplie p-8 md:p-12 text-center">
          <Building2 className="w-12 h-12 text-gray-400 mx-auto mb-4" />
